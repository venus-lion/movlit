...
public interface BookCommentJpaRepository extends JpaRepository<BookCommentEntity, BookCommentId> {
    ...
    @Query("SELECT NEW movlit.be.book.presentation.dto.BookCommentResponseDto( "
            ...
            + "(SELECT COUNT(bcc) FROM BookCommentEntity bcc WHERE bcc.bookEntity.bookId = :bookId ) AS allCommentsCount, m.memberId) "
    ...
    Slice<BookCommentResponseDto> getCommentsByBookIdAndMemberId(@Param("bookId") BookId bookId,
    ...
    @Query("SELECT NEW movlit.be.book.presentation.dto.BookCommentResponseDto( "
            ...
            + "bc.regDt, "
            + "bc.updDt,"
            + "0L AS allCommentsCount, m.memberId) "
            + "FROM BookCommentEntity bc "
            + "LEFT JOIN MemberEntity m on m.memberId = bc.memberEntity.memberId "
            + "WHERE bc.bookEntity.bookId = :bookId and m.memberId = :memberId")
    Optional<BookCommentResponseDto> fetchCommentByMemberAndBook(@Param("memberId") MemberId memberId,
