...
public interface BookJpaRepository extends JpaRepository<BookEntity, BookId> {
    ...
    @Query("SELECT DISTINCT b "
            + "FROM BookEntity b "
            + "LEFT JOIN b.bookGenreEntities bg "
            // LEFT JOIN으로 변경 -- oneToMany라서 페이징과 컬렉션조회 같이 안되서 그냥 join -> 그래서 yml에 batchsize 설정
    ...
    Optional<BookDetailResponseDto> findBookDetailByBookId(@Param("bookId") BookId bookId,
